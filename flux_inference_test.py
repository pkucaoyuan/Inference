...
class FluxInferenceTester:
    ...
    def load_model(self):
        ...
        try:
            ...
            self.pipe = None
            raise RuntimeError(f"FLUX模型加载失败: {e}")
    ...
    def run_inference_test(self, prompt, negative_prompt="", steps=50, cfg=3.5, width=1024, height=1024):
        """运行FLUX推理测试"""
        if not self.pipe:
            print("❌ 模型未加载")
            return None
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="FLUX模型推理测试工具")
    parser.add_argument("--gpu-id", type=int, default=0, help="指定使用的GPU ID")
    parser.add_argument("--model-path", default="./FLUX.1-schnell", help="FLUX模型路径")
    ...
    parser.add_argument("--cfg", type=float, default=3.5, help="CFG值")
    ...
    # 创建测试器
    tester = FluxInferenceTester(args.gpu_id, args.model_path)
